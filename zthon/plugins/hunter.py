...
plugin_category = "الصوتي"

# =========================================================== #
#                                                             𝙕𝙏𝙝𝙤𝙣
# =========================================================== #
SONG_SEARCH_STRING = "<b>╮ جـارِ البحث؏ـن المقطـٓع الصٓوتـي... 🎧♥️╰</b>"
SONG_NOT_FOUND = "<b>⎉╎لـم استطـع ايجـاد المطلـوب .. جرب البحثباستخـدام الامـر (.اغنيه)</b>"
...
@zedub.zed_cmd(
    pattern="صوتي(320)?(?:\s|$)([\s\S]*)",
    command=("صوتي", plugin_category),
    info={
        "header": "لـ تحميـل الاغـانـي مـن يـوتيـوب",
        "امـر مضـاف": {
            "320": "لـ البحـث عـن الاغـانـي وتحميـلهـا بـدقـه عـاليـه 320k",
        },
        "الاسـتخـدام": "{tr}صوتي + اسـم الاغنيـه",
        "مثــال": "{tr}صوتي حسين الجسمي احبك",
    },
)
async def _(event):
    "To search songs"
    ...
    if event.pattern_match.group(2):
        ...
        return await edit_or_reply(event, "**⎉╎قم باضافـة الاغنيـه للامـر .. صوتي + اسـم الاغنيـه**")
...
@zedub.zed_cmd(
    pattern="اصوتي(?:\ع|$)([\s\S]*)",
    command=("اصوتي", plugin_category),
    info={
        "header": "To reverse search song.",
        "الوصـف": "Reverse search audio file using shazam api",
        "امـر مضـاف": {"ع": "To send the song of sazam match"},
        "الاستخـدام": [
            "{tr}اصوتي بالــرد ع بصمـه او مقطـع صوتي",
            "{tr}اصوتي ع بالــرد ع بصمـه او مقطـع صوتي",
        ],
    },
)
...
async def you_search(event):
    ...
    if not query:
        return await edit_delete(
            event, "**╮ بالـرد ﮼؏ كلمـٓھہ للصوتي أو ضعها مـع الأمـر ... 𓅫╰**"
        )
    ...
    reply_text = f"**•  اليك عزيزي قائمة بروابط الكلمة اللتي تبحث عنها:**\n`{query}`\n\n**•  النتائج:**\n{full_response}"
